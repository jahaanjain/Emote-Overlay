...
async function getEmotes(check) {
    ...
    let res = await fetch(proxyurl + 'https://customapi.aidenwallis.co.uk/api/v1/twitch/toID/' + channel, { // using aidenwallis API for now to get twitch ID from username
        method: "GET"
...
// Main module for connecting to twitch chat
function connect() {
    ...
    chat.onmessage = function(event) {
        ...
        log(messageFull)
        if (messageFull.length == 14) {
            let messageBefore = messageFull[13].split(`${channel} :`).pop(); // gets the raw message
